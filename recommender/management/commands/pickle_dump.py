    ...
    def handle(self, *args, **options):
        ...
        if options['method'] == 'neologd':
            for spot in spots:
                ...
                if spot.id % 1000 == 0:
                    file_path = settings.BASE_DIR + "/recommender/lib/files/pickles/neologd/{}.pickle".format(data_count)
            ...
            for spot in spots:
                ...
                json_data[spot.id] = contents
                if spot.id % 1000 == 0:
                    file_path = settings.BASE_DIR + "/recommender/lib/files/pickles/jumanpp/{}.pickle".format(data_count)
                    with open(file_path, mode='wb') as f:
                        pickle.dump(json_data, f)
                    json_data = {}
                    data_count += 1
